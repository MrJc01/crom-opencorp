...
2. **Registros**: antes de encerrar, anexe o resultado da execução em `.opencorp/registries/execucoes/` (o que foi feito, artefatos produzidos, status).
3. **Documentos gerais**: se sua ordem produziu um documento de referência para outros agentes, salve-o em `.opencorp/registries/documentos/` com descrição clara.
4. **Logs referenciais**: eventos pontuais que outros agentes precisarão rastrear (bloqueios, anomalias, avisos) → `.opencorp/registries/logs/`.
5. **Segurança**: se a ordem exige algo bloqueado pela política, recuse, registre em `.opencorp/registries/logs/` e avise o humano na saída.
